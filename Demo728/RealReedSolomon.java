...
public class RealReedSolomon {
    ...
    public List<byte[]> encode(byte[] data) {
        ...
        System.out.println("Encoding " + data.length + " bytes into " + totalShards + " shards of " + shardLength + " bytes each");
        ...
        // Distribute input data across data shards
        ...
        // Calculate parity shards using matrix multiplication in GF(2^8)
        for (int parityIndex = 0; parityIndex < parityShards; parityIndex++) {
            ...
            for (int byteIndex = 0; byteIndex < shardLength; byteIndex++) {
                ...
                // Matrix multiplication: parity[i] = sum(matrix[data+i][j] * data[j])
        ...
        System.out.println("Generated " + parityShards + " parity shards using Vandermonde matrix");
        return shards;
    }
    ...
    private byte[] reconstructData(List<byte[]> shards, int shardLength) {
        ...
        for (int byteIndex = 0; byteIndex < shardLength; byteIndex++) {
            for (int shardIndex = 0; shardIndex < dataShards; shardIndex++) {
                byte b = shards.get(shardIndex)[byteIndex];
                result.add(b);
            }
        }
        
        // Convert to array and remove padding
